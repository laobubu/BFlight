...
void MPU6050_reset(void) {
    IICwriteBit(MPU6050_ADDR, MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_DEVICE_RESET_BIT, 1);
}
...
uint8_t MPU6050_getDeviceID(void) {
    ...
    IICreadBytes(MPU6050_ADDR, MPU6050_RA_WHO_AM_I, 1, buffer);
    return buffer[0];
}
...
void MPU6050AccRead(int16_t *accData)
{
    ...
    IICreadBytes(MPU6050_ADDR, MPU6050_RA_ACCEL_XOUT_H, 6, buf);
...
void MPU6050GyroRead(int16_t *gyroData)
{
    ...
    IICreadBytes(MPU6050_ADDR, MPU6050_RA_GYRO_XOUT_H, 6, buf);
...
void MPU6050_setGyroOffset(int16_t offset[3])
{
		...
		for(i=0;i<3;i++)
		{
			buf[0]=offset[i]>>8;
			buf[1]=offset[i];
			IICwriteBytes(MPU6050_ADDR, MPU6050_RA_XG_OFFS_USRH + i*2, 2,buf);
		}	 
}
...
void MPU6050_setMemoryBank(uint8_t bank, uint8_t prefetchEnabled, uint8_t userBank) {
    ...
    IICwriteByte(MPU6050_ADDR, MPU6050_RA_BANK_SEL, bank);
}

// MEM_START_ADDR register
void MPU6050_setMemoryStartAddress(uint8_t address) {
    IICwriteByte(MPU6050_ADDR, MPU6050_RA_MEM_START_ADDR, address);
}

// MEM_R_W register
uint8_t MPU6050_readMemoryByte(void) {
    IICreadBytes(MPU6050_ADDR, MPU6050_RA_MEM_R_W, 1 , buffer);
    return buffer[0];
}

// XG_OFFS_USR* registers
int16_t MPU6050_getXGyroOffset(void) {
    IICreadBytes(MPU6050_ADDR, MPU6050_RA_XG_OFFS_USRH, 2, buffer);
...
int16_t MPU6050_getZGyroOffset(void) {
    IICreadBytes(MPU6050_ADDR, MPU6050_RA_ZG_OFFS_USRH, 2, buffer);
    return (((int16_t)buffer[0]) << 8) | buffer[1];
}
...
uint8_t MPU6050_writeMemoryBlock(const uint8_t *data, uint16_t dataSize, uint8_t bank, uint8_t address, uint8_t verify, uint8_t useProgMem) {
    ...
    for (i = 0; i < dataSize;) {
        ...
        IICwriteBytes(MPU6050_ADDR, MPU6050_RA_MEM_R_W, chunkSize, tprogBuffer);

        // verify data if needed
        if (verify) {
            MPU6050_setMemoryBank(bank,0,0);
            MPU6050_setMemoryStartAddress(address);
            IICreadBytes(MPU6050_ADDR, MPU6050_RA_MEM_R_W, chunkSize, verifyBuffer);
    ...
    for (i = 0; i < dataSize;) {
        ...
        if (length > 0) {
            ...
            if (special == 0x01) {
                ...
                IICwriteByte(MPU6050_ADDR, MPU6050_RA_INT_ENABLE, 0x32);  // single operation
...
void MPU6050_setIntEnabled(uint8_t enabled) {
    IICwriteByte(MPU6050_ADDR, MPU6050_RA_INT_ENABLE, enabled);
}
...
void MPU6050_setExternalFrameSync(uint8_t sync) {
    IICwriteBits(MPU6050_ADDR, MPU6050_RA_CONFIG, MPU6050_CFG_EXT_SYNC_SET_BIT, MPU6050_CFG_EXT_SYNC_SET_LENGTH, sync);
}

void MPU6050_setDMPConfig1(uint8_t config) {
    IICwriteByte(MPU6050_ADDR, MPU6050_RA_DMP_CFG_1, config);
}
...
void MPU6050_setOTPBankValid(uint8_t enabled) {
    IICwriteBit(MPU6050_ADDR, MPU6050_RA_XG_OFFS_TC, MPU6050_TC_OTP_BNK_VLD_BIT, enabled);
}
...
void MPU6050_setXGyroOffset(int16_t offset) {
    ...
    IICwriteBytes(MPU6050_ADDR, MPU6050_RA_XG_OFFS_USRH, 2, buffer);
}
void MPU6050_setYGyroOffset(int16_t offset) {
	buffer[0] = offset>>8;
	buffer[1] = offset&0x00ff;
    IICwriteBytes(MPU6050_ADDR, MPU6050_RA_YG_OFFS_USRH, 2, buffer);
}

void MPU6050_setZGyroOffset(int16_t offset) {
    buffer[0] = offset>>8;
	buffer[1] = offset&0x00ff;
    IICwriteBytes(MPU6050_ADDR, MPU6050_RA_ZG_OFFS_USRH, 2, buffer);
}
...
void MPU6050_resetFIFO(void) {
    IICwriteBit(MPU6050_ADDR, MPU6050_RA_USER_CTRL, MPU6050_USERCTRL_FIFO_RESET_BIT, 1);
}
...
uint16_t MPU6050_getFIFOCount(void) {
    IICreadBytes(MPU6050_ADDR, MPU6050_RA_FIFO_COUNTH, 2, buffer);
...
void MPU6050_setMotionDetectionThreshold(uint8_t threshold) {
    IICwriteByte(MPU6050_ADDR, MPU6050_RA_MOT_THR, threshold);
}
...
void MPU6050_setZeroMotionDetectionThreshold(uint8_t threshold) {
    IICwriteByte(MPU6050_ADDR, MPU6050_RA_ZRMOT_THR, threshold);
}
...
void MPU6050_setMotionDetectionDuration(uint8_t duration) {
    IICwriteByte(MPU6050_ADDR, MPU6050_RA_MOT_DUR, duration);
}
...
void MPU6050_setZeroMotionDetectionDuration(uint8_t duration) {
    IICwriteByte(MPU6050_ADDR, MPU6050_RA_ZRMOT_DUR, duration);
}
...
void MPU6050_readMemoryBlock(uint8_t *data, uint16_t dataSize, uint8_t bank, uint8_t address) {
    ...
    for (i = 0; i < dataSize;) {
        ...
        IICwriteBytes(MPU6050_ADDR, MPU6050_RA_MEM_R_W, chunkSize, data + i);
...
void MPU6050_getFIFOBytes(uint8_t *data, uint8_t length) {
    IICreadBytes(MPU6050_ADDR, MPU6050_RA_FIFO_R_W, length, data);
}
...
uint8_t MPU6050_getIntStatus(void) {
    return IICreadOneByte(MPU6050_ADDR, MPU6050_RA_INT_STATUS);
}

void MPU6050_setDMPEnabled(uint8_t enabled) {
    IICwriteBit(MPU6050_ADDR, MPU6050_RA_USER_CTRL, MPU6050_USERCTRL_DMP_EN_BIT, enabled);
}

uint8_t MPU6050_getOTPBankValid(void) {
	uint8_t  temp = IICreadOneByte(MPU6050_ADDR, MPU6050_RA_XG_OFFS_TC);
...
void MPU6050_setXGyroOffsetTC(int8_t offset) {
    IICwriteBits(MPU6050_ADDR, MPU6050_RA_XG_OFFS_TC, MPU6050_TC_OFFSET_BIT, MPU6050_TC_OFFSET_LENGTH, offset);
}


// YG_OFFS_TC register
int8_t MPU6050_getYGyroOffsetTC(void) {
	uint8_t  temp = IICreadOneByte(MPU6050_ADDR, MPU6050_RA_YG_OFFS_TC);
	temp &= 0x3F;
    return temp;
}
void MPU6050_setYGyroOffsetTC(int8_t offset) {
    IICwriteBits(MPU6050_ADDR, MPU6050_RA_YG_OFFS_TC, MPU6050_TC_OFFSET_BIT, MPU6050_TC_OFFSET_LENGTH, offset);
}

// ZG_OFFS_TC register
int8_t MPU6050_getZGyroOffsetTC(void) {
	uint8_t  temp = IICreadOneByte(MPU6050_ADDR, MPU6050_RA_ZG_OFFS_TC);
	temp &= 0x3F;
    return temp;
}
void MPU6050_setZGyroOffsetTC(int8_t offset) {
    IICwriteBits(MPU6050_ADDR, MPU6050_RA_ZG_OFFS_TC, MPU6050_TC_OFFSET_BIT, MPU6050_TC_OFFSET_LENGTH, offset);
}
...
void MPU6050_setSlaveAddress(uint8_t num, uint8_t address) {
    if (num > 3) return;
    IICwriteByte(MPU6050_ADDR, MPU6050_RA_I2C_SLV0_ADDR + num*3, address);
}
...
void MPU6050_resetI2CMaster(void) {
    IICwriteBit(MPU6050_ADDR, MPU6050_RA_USER_CTRL, MPU6050_USERCTRL_I2C_MST_RESET_BIT, 1);
}
...
void MPU6050_setFIFOEnabled(uint8_t enabled) {
    IICwriteBit(MPU6050_ADDR, MPU6050_RA_USER_CTRL, MPU6050_USERCTRL_FIFO_EN_BIT, enabled);
}

void MPU6050_resetDMP(void) {
    IICwriteBit(MPU6050_ADDR, MPU6050_RA_USER_CTRL, MPU6050_USERCTRL_DMP_RESET_BIT, 1);
}
