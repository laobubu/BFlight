...
const static unsigned char SROM[1986]={
...
#define ADNS3080_RST_GPIO	GPIOF
#define ADNS3080_RST_PIN	GPIO_PIN_1

#define ADNS3080_NPD_GPIO	GPIOF
#define ADNS3080_NPD_PIN	GPIO_PIN_2

#define ADNS3080_NCS_GPIO	GPIOA
#define ADNS3080_NCS_PIN	GPIO_PIN_4
#define ON_CS()				ADNS3080_NCS_GPIO->BSRR = ADNS3080_NCS_PIN << 16
#define OFF_CS()			ADNS3080_NCS_GPIO->BSRR = ADNS3080_NCS_PIN

extern SPI_HandleTypeDef hspi1;

volatile ADNS3080_Typedef ADNS3080;

///////////////////////////PORT

static char SPI_SendReceive(char data);
...
static uint8_t read_busy(void);//写帧率的判忙
static void clear_motion(void);

char SPI_SendReceive(char data)
{
	char rtnbuffer;
	HAL_SPI_TransmitReceive(&hspi1, (uint8_t*)&data, (uint8_t*)&rtnbuffer, 1, HAL_MAX_DELAY);
	return rtnbuffer;
}
...
{
	ADNS3080_Reset();
	HAL_GPIO_WritePin(ADNS3080_NPD_GPIO, ADNS3080_NPD_PIN, GPIO_PIN_SET);  //拉高NPD,免睡眠
	delay_ms(10);
	...
	ON_CS();	
		...
		SPI_SendReceive(0xe0); //          40 3000帧率        C0 5000帧率	   
...
void ADNS3080_Reset(void)  //ADNS3080 复位（高）
{
	...
	delay_ms(5);
	HAL_GPIO_WritePin(ADNS3080_RST_GPIO, ADNS3080_RST_PIN, GPIO_PIN_SET);
	delay_ms(5);
	HAL_GPIO_WritePin(ADNS3080_RST_GPIO, ADNS3080_RST_PIN, GPIO_PIN_RESET);
}
...
void ADNS3080_Burst_Read(void)
{
	...
	x.u=SPI_SendReceive(0xFF);
	y.u=SPI_SendReceive(0xFF);
	OFF_CS();
